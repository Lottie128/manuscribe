...
export async function buildHtml(manual: ManualInput, baseDir: string): Promise<string> {
  ...
  for (const [i, s] of manual.sections.entries()) {
    ...
    const chip = (manual.hideChips || /^(part|lesson|chapter|module|appendix|unit)\b/i.test(s.title)) ? '' : `<div class="chip">Section ${i + 1}</div>`
  ...
  <section class="cover">
    ...
    <div class="eyebrow">${esc(manual.coverKicker ?? 'STEM Suite · User Handbook')}</div>
    <h1>${esc(manual.appName)}</h1>
    <div class="rule"></div>
    <div class="tagline">${esc(manual.baseUrl)}</div>
    <div class="meta">A <b>ZeroAI</b> product · ${esc(now)}${manual.whiteLabel ? '' : '<br/>Generated by manuscribe — written by Claude Code'}</div>
  </section>
  <main class="doc">
    ...
    ${manual.whiteLabel ? '' : '<p class="credit">Generated automatically from the live application by manuscribe (ZeroAI), written by Claude Code. Review for accuracy before distribution.</p>'}
  </main>
  </body></html>`
